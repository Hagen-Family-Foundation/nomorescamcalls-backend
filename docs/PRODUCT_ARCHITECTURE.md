...
Beta authorization uses a private registration link and one backend-controlled
shared four-digit access code. Registration submits the participant-entered
code to the backend, which fails closed unless `BETA_ACCESS_CODE` is configured
as exactly four numeric digits and the submitted value matches it. The code is
not persisted or returned. Beta admission has no individual invitation, SMS
response, credential issuance, redemption, or code-management lifecycle.

Telnyx outbound SMS remains available for exact-line forwarding instructions.
Live delivery requires `TELNYX_API_KEY`, `TELNYX_API_BASE_URL`,
`TELNYX_LIVE_EXECUTION=true`, `TELNYX_MESSAGING_PROFILE_ID`, and
`TELNYX_MESSAGING_FROM_NUMBER`. Provider absence or failure is recorded
truthfully. No live email provider is part of this integration.
